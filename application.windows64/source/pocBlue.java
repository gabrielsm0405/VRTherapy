...
public class pocBlue extends PApplet {
...
public void comunicacao(){
  ...
  if(!pausa){
    ...
    if(tamDados + inicioPlot < dados.size()){
      for(int c1=inicioPlot; c1 < tamDados + inicioPlot; c1++){
        ...
        if(val){
          YF = 0.9f*YF + 0.1f*dados.get(c1).y;
        ...
        if(val){
          ZF = 0.9f*ZF + 0.1f*dados.get(c1).z;
  ...
  int corVal = 0xffFFFFFF, corDer = 0xffFFFFFF;
  ...
  PARAMETROLEVANTA = PApplet.parseInt(parametroLevanta.text);
  PARAMETROPASSO = PApplet.parseInt(parametroPasso.text);
}

public void detalhamento(PVector coord){
  ...
  text("Y: " + str(coord.y), mouseX+tamX/10, posY-tamY*0.5f/3);
}
...
public void mouseWheel(MouseEvent event){
  ...
  if(!pausa && blueConectado){
    ...
    fill(255);
    text(nf((float) 50/tempoDiv*10, 1, 1) + "%", mouseX+tamX/10, posY-tamY*1.5f/3);
  }
}
...
class TextBox{  
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "pocBlue" };
    if (passedArgs != null) {
