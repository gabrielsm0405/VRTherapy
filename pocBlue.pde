...
void setup(){
  lines = loadStrings("teste_levantou_passo5_sentou.txt"); //MODIFICAÇÃO
      ...
      case ESPERARDADOS:
        ...
        if((long) millis() - intervalTest >= 5){
         ...
         if(cont == 10){
           ...
           //println("X= "+coord[0]+"  Y= "+coord[1]+"  Z= "+coord[2]+"  EMG1= "+coord[3]+"  EMG2= "+coord[4]);
           x = int(coord[0]);
           y = int(coord[1]);
           z = int(coord[2]);
           emg1 = int(coord[3]);
...
float XF=0, YF=0, ZF=0, XFANT=0, EMG1F=0, EMG2F=0;
  ...
  if(!pausa){
    ...
    if(tamDados + inicioPlot < dados.size()){
      for(int c1=inicioPlot; c1 < tamDados + inicioPlot; c1++){
        ...
        if(val){
          EMG1F = 0*EMG1F + 1*dados.get(c1).emg1;
          //emg1y[c1-inicioPlot] = dados.get(c1).emg1;
          emg1y[c1-inicioPlot] = EMG1F;
        }
        else if(der){
        ...
        if(val){
          EMG2F = 0*EMG2F + 1*dados.get(c1).emg2;
          //emg2y[c1-inicioPlot] = dados.get(c1).emg2;
          emg2y[c1-inicioPlot] = EMG2F;
        }
        else if(der){
      ...
      if(possivelPasso){
          //if((long) millis() - tempoPossivelPasso >= 500){
            if(mudanca == false){
              println("Deu um passo. Pontuação: "+str(maxEMG1)+" "+str(maxEMG2));
              maxEMG1 = 0;
              maxEMG2 = 0;
              contPassos++;
            }
